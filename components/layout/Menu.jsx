...
export default function Menu() {
  ...
  // Keyboard navigation: collapse/expand via Space or Enter on the button
  ...
  return (
    <aside
      ...
      className={`relative z-[60] bg-bg-sidebar border-r border-border h-screen flex flex-col transition-all duration-300 ease-in-out font-inter ${isCollapsed ? 'w-[88px]' : 'w-[280px]'}`}
    >
      {/* Navigation Links */}
            ...
            <div className="space-y-1">
              {group.links.map((link) => {
                ...
                return (
                  ...
                  >
                    {isActive && (
                      <div className="absolute left-[-16px] w-1.5 h-6 bg-accent rounded-r-full shadow-[0_0_15px_rgba(59,130,246,0.5)]" />
                    )}
                    ...
                    {!isCollapsed && (
                      <span className={`text-sm font-semibold transition-all duration-300 ${isActive ? 'text-accent' : 'text-text-secondary group-hover:text-text-primary'}`}> {link.name} </span>
                    )}
      ...
      <div className="p-4 mt-auto border-t border-border bg-slate-50/50">
        ...
        >
          <div className="absolute inset-0 bg-gradient-to-r from-accent/0 via-accent/[0.05] to-accent/0 translate-x-[-100%] group-hover/profile:translate-x-[100%] transition-transform duration-1000" />
