...
export async function uploadFileToBucket(supabase, bucket, path, file) {
  ...
  if (compressed && typeof window !== 'undefined') {
    const savings = Math.round((1 - uploadTarget.size / originalSize) * 100);
    const origSizeStr = originalSize > 1024 * 1024 
      ? `${(originalSize / 1024 / 1024).toFixed(2)} MB` 
      : `${(originalSize / 1024).toFixed(1)} KB`;
    const newSizeStr = uploadTarget.size > 1024 * 1024 
      ? `${(uploadTarget.size / 1024 / 1024).toFixed(2)} MB` 
      : `${(uploadTarget.size / 1024).toFixed(1)} KB`;
    ...
    if (savings > 0) {
      toast.info(
        `${fileIcon} ${typeLabel} Optimized: ${origSizeStr} ➔ ${newSizeStr} (Saved ${savings}%)`, 
        { duration: 4000 }
      );
    }
  }
